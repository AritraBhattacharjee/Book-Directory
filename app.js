...
const Mongo_str =
  "mongodb+srv://arnabmondal800:Ioekxtg27LCHJWem@books.gsfhbsy.mongodb.net/";
...
app.delete("/delete_books/:Name", (req, res) => {
  ...
  if (password === predefinedPassword) {
    Books.deleteOne({ Name: Bookname })
      .then((result) => {
        if (result.deletedCount == 0)
          res.status(404).json({ message: "Book not found!" });
        else res.json({ message: "Book successfully deleted" });
      })
      .catch(() => {
...
app.get("/search_books", (req, res) => {
  ...
  Books.find({ Name: new RegExp(query, "i") })
    .then((results) => {
      res.json(results);
    })
    .catch(() => {
